...
bool Notifications::GetNotificationFromDictionary(
  ...
  if (!GetStringFromDictionary(kNotificationIdKey, dictionary,
      &notification_info.id)) {
    return false;
  }

  if (!GetStringFromDictionary(kNotificationCreativeSetIdKey, dictionary,
      &notification_info.creative_set_id)) {
    return false;
  }

  if (!GetStringFromDictionary(kNotificationCategoryKey, dictionary,
      &notification_info.category)) {
    return false;
  }

  if (!GetStringFromDictionary(kNotificationAdvertiserKey, dictionary,
      &notification_info.advertiser)) {
    return false;
  }

  if (!GetStringFromDictionary(kNotificationTextKey, dictionary,
      &notification_info.text)) {
    return false;
  }

  if (!GetStringFromDictionary(kNotificationUrlKey, dictionary,
      &notification_info.url)) {
    return false;
  }

  if (!GetStringFromDictionary(kNotificationUuidKey, dictionary,
